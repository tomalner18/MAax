...
class Builder:
    ...
    def build_object(self, obj_type, mass, pos, rot, vel, ang, scale, interact=False):
        ...
        if obj_type == "box":
            return Box(mass, pos, rot, vel, ang, scale, interact)
        elif obj_type == "sphere":
            return Sphere(mass, pos, rot, vel, ang, scale, interact)
        elif obj_type == "plane":
            return Plane(mass, pos, rot, vel, ang, scale, interact)
        elif obj_type == "clipped_plane":
            return ClippedPlane(mass, pos, rot, vel, ang, scale, interact)
        elif obj_type == "capsule":
            return Capsule(mass, pos, rot, vel, ang, scale, interact)
        else:
    ...
    def build(self, objects, obj_type):
        ...
        for obj in objects:
            pos = [obj[0], obj[1], 1]
            built_objects.append(self.build_object(obj_type, mass, pos, rot, vel, ang, scale))
